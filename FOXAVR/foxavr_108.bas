...
'to-do-list: SER-Eingabe keying speed 48, 54, 60, 72, 90 BpM
'            + Test-Phase
...
Cticks:                                                     'number of ticks = dots per second
  Data 8                                                    'useful values 8 = 48; 9 = 54; 10 = 60; 12 = 72; 15 = 90 BpM
Ccalib:                                                     'timer compare value for calibration
  Data 57600%                                               '57600± @ 8; 51200± @ 9; 46080± @ 10; 38400± @ 12; 30720± @ 15 ticks
...
'MOE to MO5 are beacons according to the international standard
...
Const Keyspeedbase = 204                                    '57600± @ 8; 51200± @ 9; 46080± @ 10; 38400± @ 12; 30720± @ 15 ticks
Data 8 , 57600 , 0                                          '48 BpM
Data 9 , 51200 , 0                                          '54 BpM
Data 10 , 46080 , 0                                         '60 BpM
Data 12 , 38400 , 0                                         '72 BpM
Data 15 , 30720 , 0                                         '90 BpM
...
Dim Keyspeed As Byte                                        'keyspeed in bpm
...
Dim Keylen As Byte                                          'keying code size
...
Print "FOXAVR 1.08 - www.DL6DBN.de - (CC BY-NC-SA 3.0)"
If Ser_dip_pin = 0 Then                                     'parameters via serial port (1) or dip switch (0)
  If Programpin = 0 Then                                    'Programming selected?  In reality (UART-Pin) = 1
    ...
    If Cycle > 20 Then                                      'more than available?
      Print "ERROR: to many beacons"                        'yes, error message
    ...
    If Cycle > 1 Then
      ...
      If Active > Cycle Then                                'larger than number of beacons?
        Print "ERROR: doesn't match number of beacons"      'yes, error message
        Active = 0                                          'set to a useful default
      End If
      Print "Seconds of active transmitting slot (10 - 240)"
      ...
      If Slot < 10 Or Slot > 240 Then                       'less than 10 seconds or more than 4 minutes? I think, it makes no sense
        Print "ERROR: slot time too low or too high"        'yes, error message
    ...
    Else
      ...
      Slot = 60
    End If
    Print "Keying speed (48, 54, 60, 72, 90 BpM)"
    Input "?>" , Keyspeed
'ADD code to set maxticks and timer calibration
    ...
    If Wait_hr >= 36 Then                                   'larger than 36 = time error too high
      Print "ERROR: waiting hours too high"                 'yes, error message
    ...
    If Wait_min >= 60 Then                                  'larger than 59
      Print "ERROR: waiting minutes too high"               'yes, error message
...
Else                                                        'parameters via DIP switch
  ...
  End If
  Readeeprom Maxticks , Cticks                              'get keying speed from EEPROM
...
Print Cycle ; "/" ; Active ; " " ; Slot ; "s " ; Keyspeed ; "BpM " ; Wait_hr ; ":" ; Wait_min
...
End If
Incr Temp
Keylen = Keylen + Keycodes(temp)                            'number of keying ticks
Incr Temp
Key(4) = Keycodes(temp)
Incr Temp
Key(5) = Keycodes(temp)
...
Txon:
  ...
  If Keylen = Keyptr Then                                   'key code completed?
    Keyptr = 0                                              'yes, start again
  End If
Return

Teston:                                                     'transmitter during test on
    Print "test on"
    Temp1 = 0
  'Call Txon
Return