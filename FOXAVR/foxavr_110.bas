...
Cwloport Alias Portd.4
Cwhiport Alias Portd.6
Txhiport Alias Portd.3
Txloport Alias Portd.2
Prgport Alias Portd.0
Programpin Alias Pind.0

Toneport Alias Portb.2
Uartrxon Alias Ucsrb.4
Uarttxon Alias Ucsrb.3

'definition of constants

Const Hr_min = 60                                           'a minute has 60 seconds and an hour 60 minutes
Const Ledflash = 10                                         'seconds between flashing of LED
Const Bitbyte = 8                                           '8 bits are 1 byte

'EEPROM contains main parameters, tables and keying code

$eeprom                                                     'blank due to chip error compatibility
  Data 0
...
Reset Uartrxon                                              'UART-RX off
Reset Prgport                                               'hi-z on program pin
Config Cwloport = Output
Config Cwhiport = Output
Config Txhiport = Output
Config Txloport = Output
Config Prgport = Input

Set Txloport
Reset Txhiport                                              'tx off
Set Cwloport                                                'keying off
Reset Cwhiport
Config Toneport = Input                                     'tone off

If Programpin = 1 Then                                      'Programming selected?
  Set Uartrxon                                              'UART-RX on
  Echo On                                                   'yes
  ...
  If Active > Cycle Then                                    'larger than number of beacons?
    Active = Cycle                                          'set to a useful default
  End If
  ...
  Select Case State
    Case 0:                                                 'waiting state
      If Hour = Wait_hr And Minute = Wait_min Then          'waiting time over?
        ...
        If Cycle < 2 Then                                   'permanent beacon?
          State = 3                                         'yes, state = 3
          Reset Txloport                                    'and tx on, one tick before keying
          Set Txhiport
        Else
          State = 1                                         'no, state = 1
        End If
    ...
    Case 1:                                                 'inactive state
      If Minute = Active Then                               'active time reached?
        ...
        Keyptr = 0
        Reset Txloport
        Set Txhiport                                        'and tx on, one tick before keying
      Else
        Istate = 0
    ...
    Case 2:                                                 'active state
      If Minute <> Active Then                              'active time over?
        ...
        Set Cwloport                                        'and tx off immediately
        Reset Cwhiport
        Config Toneport = Input                             'tone off
        Set Txloport
        Reset Txhiport
