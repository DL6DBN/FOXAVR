...
Ccalib:                                                     'timer compare value for calibration
  Data 57601%                                               '57600± @ 8; 51200± @ 9; 46080± @ 10 ticks
...
Const Waitbase = 176                                        'waiting times (hrs, min) for DIP parameters
Data 0 , 0
Data 11 , 0
Data 0 , 2
Data 0 , 3
...
If Ser_dip_pin = 1 Then                                     'parameters via serial port (1) or dip switch (0)
  If Programpin = 1 Then                                    'Programming selected?  In reality (UART-Pin) = 1
...
'configure and preset timer1 in compare and clear mode for clock of state machineConfig Pinb.1 = Input                                       'preparing port for DIP switch input


'Config Timer1 = Timer , Prescale = 8 , Clear Timer = 1 , Compare A = Disconnect
TCCR1A = &B00000000   'manual timer settings to overcome compiler error...
TCCR1B = &B00001010   '...setting OCR1-Pin to output HI
Config Pinb.1 = Input                                       'preparing port for DIP switch input
Compare1a = Tim
